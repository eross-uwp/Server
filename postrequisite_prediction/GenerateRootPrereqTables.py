...
__OUTPUT_CSV_FILEPATH = 'data\\RootPrereqTables\\'


class GenerateRootPrereqTables:
    ...
    # Creates a data frame for a postrequisite as the title and adds all the root prerequisite courses as columns along
    # with: cumulative gpa, previous term gpa, struggle, and term difference.
    def create_data_frame(self, tree, grades):
        ...
        for k in list(prerequisite):
            if k.does_have_prereq() == 1:
                prerequisite.remove(k)
...
if __name__ == "__main__":
    ...
    postreqquisite_lrm = GenerateRootPrereqTables()
    count = 1
