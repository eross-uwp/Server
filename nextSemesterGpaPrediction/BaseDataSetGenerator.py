...
def get_term_pairs(raw_data):
    ...
    for id in headers:  # for every student id in the raw dataset
        terms = []
        for termNumber in raw_data[id].index:  # for every term row of an id column
            if raw_data[id].loc[termNumber] > 0:
        ...
        if len(terms) > 1:  # as long as a student id is related to at least 2 terms of non-zero gpa
            ...
            data_frame_for_pairs[id] = [first_term, second_term]  # random term and the term previous to it for each id
